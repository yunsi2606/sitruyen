...
export default factories.createCoreService('api::chapter.chapter', ({ strapi }) => ({
    async readChapter(chapterId: string | number, user: StrapiUser | null, ip: string) {
        ...
        if (!lastViewTime || (now - lastViewTime) > CACHE_DURATION) {
            ...
            const chapterTableName = strapi.db.metadata.get('api::chapter.chapter').tableName;
            const storyTableName = strapi.db.metadata.get('api::story.story').tableName;

            // Use raw knex connection to update view_count without triggering lifecycles or bumping updatedAt
            await strapi.db.connection(chapterTableName)
                .where({ id: chapter.id })
                .update({ view_count: newChapterViews });
            ...
            if (chapter.story) {
                ...
                await strapi.db.connection(storyTableName)
                    .where({ id: chapter.story.id })
                    .update({ view_count: newStoryViews });
            }
